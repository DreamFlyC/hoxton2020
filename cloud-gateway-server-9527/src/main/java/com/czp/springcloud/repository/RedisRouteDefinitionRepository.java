 ...
 * @author : CZP
 * @version :
 * @date : Created in 2020-3-17 10:38:54
 * @description : 自定义redis存储路由配置信息，实现动态路由，将定义好的路由表信息通过此类读写到redis中
...
public class RedisRouteDefinitionRepository implements RouteDefinitionRepository {
	...
	public RedisRouteDefinitionRepository(RedisTemplate<String, Object> redisTemplate, StringRedisTemplate stringRedisTemplate) {
	...
	public Mono<Void> save(Mono<RouteDefinition> route) {
		lock.writeLock().lock();
		Mono<Void> objectMono;
		try {
			objectMono = route.flatMap(r -> {
